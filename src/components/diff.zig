...
pub const DisplayLine = struct {
    ...
    fn renderFileHeader(self: DisplayLine, plane: *c.ncplane, y: c_int, text_x: c_int) !void {
        ...
        if (self.old_file_path) |old_path| {
            ...
            if (renamed) {
                ...
                const old_visible = util.clipToDisplayWidth(old_path, text_available);
        ...
        const text_available: usize = @intCast((end_x - x) -| 1);
        const visible_text = util.clipToDisplayWidth(self.text, text_available);
    ...
    fn putSegment(plane: *c.ncplane, y: c_int, x: c_int, text: []const u8) !void {
        ...
        const clipped = util.clipToDisplayWidth(text, available_cols);
